...
class MyTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_amazon_web_agent(self):
        ...
        amazon_web_agent_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """
                    You are a specialized assistant tasked on performing actions on Amazon web page.
                    After you find the web elements that the user requested, use 'extract_content' to extract the content.
                    Remember that the task isn't completed until after the relevant tool has successfully been used. 
        ...
        inputs = {
            "messages": [
                HumanMessage(content="Show me my shopping cart info on Amazon")
            ]
        }
